...
export class sentimentWords extends Component {
  ...
  render() {
    return (
      ...
      <h2>Positive:</h2>
      <ul className="positive-words">{this.renderWords(this.findMostFrequentWords(this.props.positiveWords))}</ul>
      <h2>Negative:</h2>
      <ul className="negative-words">{this.renderWords(this.findMostFrequentWords(this.props.negativeWords))}</ul>
      </div>
    )
  }
}
